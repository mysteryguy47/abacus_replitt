...
export default function PaperCreate() {
  ...
  useEffect(() => {
    // Only load presets for AB-1 through AB-6 levels (only available on Basic page)
    if (isBasicPage && level.startsWith("AB-")) {
      // Load preset blocks from backend
      fetch(`/api/presets/${level}`)
        .then(res => res.json())
        .then(data => {
        ...
        .catch(err => {
          console.error("Failed to load preset blocks:", err);
    ...
    // Reset to Custom if on Junior/Advanced page and level is AB-X
    if ((isJuniorPage || isAdvancedPage) && level.startsWith("AB-")) {
      setLevel("Custom");
      setBlocks([]);
    }
    // Note: Junior and Advanced pages only allow Custom level
